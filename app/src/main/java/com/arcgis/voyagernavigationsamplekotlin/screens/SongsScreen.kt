...
import com.arcgis.voyagernavigationsamplekotlin.data.getSongs

object SongsScreen : Screen {
    @Composable
...
fun SongsScreen() {
    ...
    Column(
        ...
        val songsList: List<Songs> = getSongs()
        Column(modifier = Modifier.padding(12.dp)) {
            LazyColumn(modifier = Modifier.fillMaxHeight().padding(bottom = 20.dp)) {
                items(items = songsList, itemContent = { item ->
                    Row(
                        modifier =
                        Modifier
                            .padding(10.dp)
                            ...
                            .selectable(
                                selected = true,
                                onClick = { navigator.push(SongsDetailsScreen(item)) })
                    ) {
                        AsyncImage(
                            model = ImageRequest.Builder(LocalContext.current)
                                .data(item.artworkUrl60)
                                .crossfade(true)
                        ...
                        Column(
                            ...
                            Text(text = item.wrapperType?: "", color = Color.Black, fontSize = 14.sp)
                            Spacer(Modifier.height(2.dp))
                        }
                    }
